...
class KasirController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        return view('dashboard.kasir.kasir-index', compact('roti', 'total_penjualan'));
    }


    public function indexCustomer(Request $request)
    {
        $roti = KatalogRoti::with('resepRoti')->get();
        ...
        if (request()->ajax()) {
            if (!empty($request->from_date)) {
                ...
                $data = DB::table('kasirs')
                    ...
                    ->get();
                $total_penjualan = Kasir::where('tanggal_diproduksi', $now)->sum('total_penjualan_ini') ?? 0;
                $total_pemesanan = Kasir::where('tanggal_diproduksi', $now)->sum(DB::raw('total_rizky + total_palem + total_moro_jaya')) ?? 0;
                $total_ppn = Kasir::where('tanggal_diproduksi', $now)->sum('total_ppn') ?? 0;
                $total_toko = Kasir::where('tanggal_diproduksi', $now)->sum('total_toko') ?? 0;
                $total_after_ppn = Kasir::where('tanggal_diproduksi', $now)->sum('total_after_ppn') ?? 0;
            }
        ...
        return view('dashboard.kasir.kasir-customer', compact('roti', 'total_penjualan'));
    }
    ...
    public function indexPemesanan(Request $request)
    {
        ...
        return view('dashboard.kasir.kasir-pemesanan', compact('roti', 'total_penjualan'));
    }

    public function getDataRoti(Request $request)
    {
        $data['roti'] = ResepRoti::with(['ProduksiRoti', 'katalogRoti'])->where('nama_resep_roti', $request->nama_resep_roti)->first();
    ...
    public function storeCustomer(Request $request)
    {
        ...
        $resep = ResepRoti::with(['bahanBaku', 'katalogRoti'])->where('nama_resep_roti', $request->nama_roti)->first();
        $produksiRoti = KatalogRoti::with('resepRoti')->find($request);
        ...
        // Inisiasi Sisa
        $sisa_total = $resep->katalogRoti->stok - $request->laku;
        ...
        if ($kasir) {
            if ($resep->katalogRoti->stok  == 0 || $resep->katalogRoti->stok < $request->laku) {
                ...
                if ($request->pemesanan == null) {
                    $resep->katalogRoti->stok = $request->input('sisa');
                    $resep->katalogRoti->laku += $request->input('laku');
                    $resep->katalogRoti->save();
                }
            }
        }
    ...
    public function hitungTotalPenjualan($request, $resep)
    {
        $pemesan = '';
    ...
    public function store(Request $request)
    {
        ...
        if ($kasir) {
            ...
            if ($request->pemesanan == 'Rizky' || $request->pemesanan == 'Palem' || $request->pemesanan == 'Moro Jaya') {
                ...
                $kasir->nama_roti = $nama_resep_roti;
                // Stok Masuk = Stok Inisiasi pada hari itu
                $kasir->stok_masuk = $stokMasuk;
                $kasir->stok_sekarang = $resep->stok_sekarang;
                $kasir->laku = $kasir->laku += $laku;
                $kasir->roti_off = $request->roti_off ?? 0;
